...
> 一站式运维工作站 —— SSH 终端、SFTP 文件管理、MySQL 数据库、远程桌面、AI 智能助手、MCP 服务，全部在一个应用里。
...
多标签终端，支持会话分组管理。右键菜单（复制/粘贴/搜索/清屏），选中即复制，Ctrl+F 终端搜索，连接断开一键重连。会话支持**拖拽移动到不同分组**，快捷命令栏支持**分组标签页**按类别筛选。
...
本地 / 远程双栏浏览，面包屑导航，列排序，多文件批量传输，传输进度实时显示。支持**面板间拖拽传输**——直接将文件从本地栏拖到远程栏上传，或从远程栏拖到本地栏下载。
...
类 mysql CLI 命令行模式（回车执行、历史保留），或切换到代码模式（多行编辑器 + 结果表格）。左侧数据库树（**分组 → 实例 → 库 → 表**），展开实例自动连接，点击表名自动补全字段，结果可导出 CSV / JSON。支持连接分组管理（创建/重命名/删除分组），实例节点悬浮菜单快速编辑/删除。
...
将 X-Term 的执行能力通过标准 **MCP（Model Context Protocol）** 暴露给外部 AI 客户端（Claude Desktop、Cursor、Windsurf 等），让外部 AI 直接操作你的服务器和数据库。
...
特性：
- 基于 HTTP + SSE 传输（兼容 MCP 2024-11-05 协议）
- Bearer Token 认证，默认绑定 `127.0.0.1`
- **人工确认机制**：外部 AI 调用 exec 工具时弹出审批浮层，你确认后才执行
- 可开启自动放行（跳过确认，适合受信环境）
- 实时执行日志面板
- 一键生成客户端配置 JSON

<!-- MCP 截图 -->
![MCP 服务端](docs/screenshots/mcp.png)
...
设置 → 关于 中可检查更新：从自建服务器的 `update.json` 拉取版本清单，发现新版后流式下载安装包（实时进度、sha256 完整性校验），确认后拉起安装器完成升级。更新源地址可在页面内配置。清单格式见 [`docs/update.example.json`](docs/update.example.json)。
