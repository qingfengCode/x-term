...
REM  产物：src-tauri\target\release\bundle\ 下的 msi / nsis 安装包
REM
...
REM 1. 加载 MSVC 编译环境（和 run-dev.bat 一致，避免 cc 用到 MinGW gcc）。
...
REM 3. 安装前端依赖（如果 node_modules 不存在或 lockfile 变化）。
...
REM 4. 执行 Tauri 构建（release + 打包 msi/nsis）。
...
echo [build] === 构建成功！ ===
echo [build] 安装包位置：
echo   D:\code\tanghan-yunwei\x-term\src-tauri\target\release\bundle\msi\
