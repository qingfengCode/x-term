...
impl Session {
    ...
    async fn run_inner<H: Handler + Send, R: AsyncRead + AsyncWrite + Unpin + Send>(
        ...
        while !self.common.disconnected {
            ...
            // 仅当"发送了 keepalive"时才重置 keepalive 定时器，**不因服务器有
            // 数据而重置**——与 OpenSSH 的 ServerAliveInterval 语义一致：按固定
            // 间隔主动发包，服务器能持续看到客户端的数据流。部分服务器
            // （如 JumpServer koko）依赖"客户端是否有数据"判定会话存活，
            // 原逻辑（收到服务器数据就重置定时器）会让客户端永不主动发包，
            // 表现为"挂机 30 秒被踢下线"。
            if sent_keepalive {
