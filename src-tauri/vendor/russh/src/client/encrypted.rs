            ...
            Some(&msg::CHANNEL_REQUEST) => {
                ...
                match req {
                    ...
                    b"keepalive@openssh.com" => {
                        ...
                        if wants_reply == 1 {
                            if let Some(ref mut enc) = self.common.encrypted {
                                // 回包必须用该 channel 的 recipient_channel
                                // （服务端视角的 id），而不是请求里的 channel_num
                                // （客户端视角的 id）。x/crypto/ssh 服务端收到
                                // CHANNEL_SUCCESS 时按 localId 查 channel，用客户端
                                // id 回包会匹配不到 → "invalid channel" 错误 →
                                // 整个连接被关闭（JumpServer koko 挂机 30 秒断线
                                // 的根因；russh 的 channel id 从 1 起分配，客户端
                                // id 与服务端 localId 通常不一致）。
                                let recipient = enc
                                    .channels
                                    .get(&channel_num)
                                    .map(|c| c.recipient_channel)
                                    .unwrap_or(channel_num.0);
                    ...
                    _ => {
                        ...
                        if wants_reply == 1 {
                            if let Some(ref mut enc) = self.common.encrypted {
                                // 与 keepalive 同理：CHANNEL_FAILURE 也必须回
                                // 服务端视角的 recipient_channel，否则服务端按
                                // localId 匹配不到响应而关闭连接。
                                let recipient = enc
                                    .channels
                                    .get(&channel_num)
